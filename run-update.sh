...
app.post('/api/fetch-worker/claim', (req, res) => {
  ...
  if (!token || token !== process.env.FETCH_WORKER_TOKEN) {
    return res.status(401).json({ error: 'Unauthorized worker token' });
  }
  if (pendingJobs.length === 0) return res.json({ job: null });
  res.json({ job: pendingJobs.shift() });
});

const uploadWorker = multer({ dest: 'uploads/' });
