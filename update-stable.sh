...
# 1. Update package.json (Bersih tanpa dependency yt-dlp yang sering error di cloud)
...
# 2. Update server.js dengan sistem fetch oEmbed stabil & audio processing
cat << 'EOF' > server.js
...
# 4. Update client/index.html dengan UI modern, tombol file estetik & back button
...
# 6. Jalankan push otomatis ke GitHub
./git-push.sh
