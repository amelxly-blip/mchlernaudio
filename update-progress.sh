...
<body>
    ...
    <div id="tab-tutorial" class="section-view">
      <div class="card">
        ...
        <ol style="padding-left: 20px; color: var(--muted); font-size: 14px; line-height: 1.8;">
          ...
          <li>Masukkan URL YouTube/TikTok, lalu klik <strong>Fetch</strong> untuk melihat progress bar.</li>
          <li>Pilih kecepatan template dan proses bypass audio Anda dengan mulus!</li>
        </ol>
      </div>
    </div>
  </main>
  ...
  <script>
    ...
    async function fetchMedia() {
      ...
      progStatus.innerText = 'Mengirim antrean ke server... Pastikan worker.py aktif di Termux!';

      try {
        progressBar.style.width = '50%';
        progStatus.innerText = 'Menunggu desktop worker mendownload audio...';
        ...
        if(data.success) {
          ...
          document.getElementById('fetch-info').innerText = `${data.metadata.sourcePlatform} • Selesai via Worker`;
          showToast('Berhasil Fetch Audio via Worker!');
        } else {
          progressBar.style.width = '100%';
          progressBar.style.background = 'var(--danger)';
          progStatus.innerText = 'Gagal: ' + (data.error || 'Unknown error');
          showToast('Gagal fetch: ' + (data.error || 'Cek worker Termux Anda!'));
        }
      } catch (err) {
        progressBar.style.width = '100%';
        progressBar.style.background = 'var(--danger)';
        progStatus.innerText = 'Error koneksi ke server: ' + err.message;
