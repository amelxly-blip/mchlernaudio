...
# Perbarui client/index.html agar tombol proses bypass bisa menggunakan hasil fetch URL
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    ...
    #progress-bar { width: 0%; height: 8px; background: var(--accent); border-radius: 6px; transition: width 0.4s ease; }
...
<body>
  <aside>
    ...
    <div style="font-size: 11px; color: var(--muted); border-top: 1px solid var(--border); padding-top: 15px;">Status: Connected</div>
  </aside>
  ...
  <script>
    ...
    function updateFileName(input) {
      ...
      if (input.files && input.files[0]) {
        ...
        fetchedAudioUrl = ''; // Reset fetch url kalau pakai file lokal
      }
    }
    ...
    async function fetchMedia() {
      ...
      progStatus.innerText = 'Mengirim antrean ke server...';
      ...
      try {
        ...
        if(data.success) {
          ...
          progStatus.innerText = 'Fetch & Download Berhasil!';
...
# 2. Perbarui juga server.js untuk mendeteksi fetchedUrl dari hasil fetch worker
cat << 'EOF' > server.js
...
git add .
git commit -m "Support fetched audio URL processing in bypass engine"
